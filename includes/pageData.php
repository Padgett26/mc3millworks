...
if ($show == "TelevisionCommercials") {
    ...
    <?php
} elseif ($show == "MC3MillworksCommercial") {
    ?>
    <iframe width="560" height="315" src="https://www.youtube.com/embed/xmL4Vz6KRYA" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
    <?php
} else {
    ...
    if ($loggedIn == 1) {
        ...
        if (filter_input(INPUT_POST, 'picUp', FILTER_SANITIZE_STRING)) {
            ...
            if ($delPic == '1') {
                $dPic1 = $db->prepare(
                        "SELECT picName, picExt, picOrder FROM pics WHERE id = ?");
                $dPic1->execute(array(
                        $picUpId
                ));
